...
* 希尔排序
  * 流程
    ...
    def shell_sort(A: list[float]):
        ...
        while d >= 1:
            # 从步长d开始遍历. 因为插入排序是从第2个元素开始的, 第一个元素下标是0
            # i是1步长递增的, 并发地处理每个子表
...
* 冒泡排序
  ...
  * 性能
    * 空间复杂度：$O(1)$
    * 时间复杂度
      * 最好：本来就有序。对比 $n-1$ 次直接结束算法，交换 $0$ 次
...
* 快速排序
  * 流程
    ...
    * 第一趟归位元素`a`，若`a`在数组中间，则第二趟对**剩下的所有元素组成的在`a`两端的左右两个区间**处理，第二趟归位元素`b, c`。前两趟共计归位 $3$ 个元素
    ...
    def partition(A: list[float], low: int, high: int) -> int:
        ...
        while low < high:
            # 找到右区间第一个 <pivot的元素, 填到上一步在 low剩的坑, high就剩出来一个坑
            # 不带等号在遇到 A=[1, 1, 1]这样的重复列表会死循环. 下同
            while A[high] >= pivot and low < high:
                high -= 1
            A[low] = A[high]

            # 找到左区间第一个 >pivot的元素, 填到上一步在 high剩的坑, low就剩出来一个坑
  ...
  * 性能
    ...
    * 时间复杂度
      ...
      |最好|最坏|平均|
      |:-:|:-:|:-:|
      |$O(n\log_2{n})$|$O(n^2)$|$O(n d)\stackrel{接近}{\to} O(n\log_2{n})$|
...
* 堆排序
  * 流程
    ...
    ```python
    # 调整大顶堆
    def heap_adjust(A: list[float], root_idx: int, left_len: int) -> list[float]:
        # 保存一下当前待调整的根元素
        temp = A[root_idx]
        ...
        # 确保下坠过程不越界
        while next_root_idx < left_len:
            # 找出最大的孩子, 同时也可能是下一步下坠到的根结点
            right_child_idx = left_child_idx + 1
            if right_child_idx < left_len and A[right_child_idx] > A[left_child_idx]:
  ...
  * 性能
    ...
    * 时间复杂度：$O(n\log_2{n})=O_{建堆}(n)+O_\text{排序}(n\log_2{n})$
      ...
      * 调整堆：$O_\text{调整}(\log_2{n})$
        * 每次调整最多 $h-i$ 层，且 $h=\lceil\log_2{(n+1)}\rceil$，因此时间复杂度为 $\log_2{n}$ 级
...
* 多路归并
  * 流程
    ...
    * 初始生成 $r$ 个归并段(内部排序成有序的初始归并段)，然后做 $s$ 次 $k$ 路归并
      ...
      * 输出缓冲区满时则写回到外存
  * 性质
    * 若有 $N$ 个记录，内存工作区可容纳 $L$ 个记录，则始生成个归并段数量 $r=\dfrac{N}{L}$
...
* 最佳归并树
  * 流程
    ...
    ***最佳归并树用于减少 $k$ 路归并时的 $\text{IO}$ 次数吗，本质就是构建 $k$ 叉哈夫曼树***</br>

    每个初始归并段视作一个叶结点(即 $0$ 度结点)，归并段内的块数作为其叶子的权值，计算是否需要补充虚段。随后构建严格 $k$ 叉哈夫曼树，即为最佳归并树

    > 初始结点数量足够，即 $(5-1)\bmod(2-1)=0$，不需要补充虚段，直接构建 $2$ 叉哈夫曼树
    > ![最佳2叉归并树](../../resource/image/data_struct/chapter6_sort_best_tree_2.jpeg "最佳2叉归并树")
    >
    > 初始结点数量不够，即 $(8-1)\bmod(3-1)=1$，需要补充 $(3-1)-1$ 个虚段，补充完后构建 $3$ 叉哈夫曼树
