...
* 快速排序
  * 流程
    ...
    每次选取一个枢轴元素确定其最终位置，该元素左边区间内均小于它，右边区间内均大于它，再对枢轴元素两侧区间递归进行上述操作</br>
...
* 堆排序
  * 流程
    ...
    >
    > 重复该操作，直到最大的 $n-1$ 个元素全部归位，自然最小的元素在堆顶。堆排序结束
    >
    > $$
    > \boxed{\begin{aligned}
  ...
  * 堆的插入删除
    ...
    * 删除
      ...
      * 先比较一次获得最大(小顶堆要最小)孩子，将放过来的堆底元素与其最大(或最小)的孩子比较，若不满足大顶堆(或小顶堆)的定义，则与最大(或最小)的孩子交换，下坠到下一层
      * 继续向下比较直至堆符合定义
    * 说明
...
* 多路归并
  ...
  * 性质
    ...
    * 多路归并花费时间 $=$ 外存 $\text{I/O}$ 时间 $+$ 内部排序时间 $+$ 内部归并时间
      * 外存 $\text{I/O}$ 时间 $=($ 文件块数 $\times 2\times(1+$ 归并趟数 $))\times$ 单次 $\text{I/O}$ 时间
      * 内部排序时间、内部归并时间一般远小于外存 $\text{I/O}$ 时间，可忽略a
...
* 置换—选择排序
  * 流程
    ...
    * $②$ 从 $\text{WA}$ 中选出关键字最小的记录，记为 $\text{MINIMAX}$ 记录。
    ...
    * $⑥$ 重复步骤 $③$ 到 $⑤$，直至在 $\text{WA}$ 中选不出新的 $\text{MINIMAX}$ 记录为止，由此得到一个初始归并段，输出一个归并段的结束标志到 $\text{FO}$ 中去。
    * 重复步骤 $②$ 到 $⑥$，直至 $\text{WA}$ 为空。由此得到全部初始归并段。
...
* 最佳归并树
  * 流程
    ...
    ***最佳归并树用于减少 $k$ 路归并时的 $\text{I/O}$ 次数，本质就是构建 $k$ 叉哈夫曼树***</br>
  ...
  * 性质
    * 归并过程的 $\text{I/O}$ 次数 $=2\times\text{WPL}$
    * $k$ 叉归并树一定是严格 $k$ 叉树，其中只有 $0$ 度结点和 $k$ 度结点，可得

        $$
        \begin{cases}
          n=n_0+n_k\\
          n=0\cdot n_0+k\cdot n_k+1
        \end{cases}
        \Rightarrow
        n_k=\dfrac{n_0-1}{k-1}
        $$
