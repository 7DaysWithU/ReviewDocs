...
* 存储
  ...
  * 共享栈(特殊的顺序栈)
    ...
    * 栈满：`top_0 + 1 == top_1`
  * 链栈
* ***⚠️卡特兰数：第 $n$ 个卡特兰数 $K_n=\dfrac{1}{n+1}C^n_{2n}$***
  * 当 $n$ 个不同元素入栈时，出栈元素不同排列个数为 $K_n$
  * 有 $n$ 个结点的不同二叉树的数量为 $K_n$
  * 由 $n$ 对括号组成的合法序列的数量为 $K_n$
  * 在一个 $n\times n$ 的网格中，从左下角走到右上角，只能向上或向右走，但要求路径不能越过从起点到终点的对角线(即任何时候都不能位于对角线上方)。这样的路径数量为 $K_n$
* ***括号匹配***
  * 初始设置空栈，顺序读入括号
  * 如果读到左括号直接入栈
...
* ***后缀表达式及其计算***
  * 后缀表达式构建
    ...
    * 遇到操作符，持续弹出栈内元素，直到栈顶元素是左括号或者栈顶元素的运算优先级小于当前运算符的优先级或者栈空。再把当前操作符入栈
      ...
      > 把"需要弹出直到栈顶 $<$ 新运算符"反过来说，就是要把优先级 $\geq$ 新运算符的先弹出去执行，这些运算符要么就是运算优先级高的，要么就是运算优先级与新运算符相同但可以先执行的
