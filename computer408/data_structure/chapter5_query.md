...
* 顺序查找
  * 定义：从头到尾遍历对比
  * $\text{ASL}_{成功}=\displaystyle\sum^n_{i=1}{\left(\dfrac{1}{n}\cdot i\right)}=\dfrac{n+1}{2},\quad\text{ASL}_{失败}=n+1$
  * 优化
    * 有序化查找表，搭配判定树查找
...
* 折半查找
  ...
  * 判定树(本质为二叉排序树)
    ...
    * 构造折半查找判定树
      ...
      > ***给出 $n$ 个结点，要求构造折半查找判定树 ( 2017 统考 T8)***

      * 首先画出有 $\lfloor\log_2{n}\rfloor$ 个结点的满二叉树，即要求 $2^{h}-1\leq n\leq 2^{h+1}-1$，其中 $h$ 为满二叉树的层高
      * 构造完 $h$ 层的满二叉树后，剩余 $n-(2^h-1)$ 个结点。按 $\text{mid}$ 取值的特点操作

        > 以 $\text{mid}$ 向下取整(标准`int`特性)为例，有：
        >
        > 对奇数个结点来说，如`A = [0, 1, 2, 3, 4]`，$\text{mid}=2$，则`A`以元素`2`分界，判定树左右子树各拥有 $2$ 个元素，左右子树的元素个数相等。对偶数个结点来说，如`A = [0, 1, 2, 3, 4, 5]`，$\text{mid}=2$，以元素`2`为分界，判定树左子树有 $2$ 个结点，右子树有 $3$ 个结点。因此，$\text{mid}$ 向下取整时，判定树任意结点一点满足其中一条性质
        >
        > $$
        > \begin{align*}
        >   左子树结点个数=右子树结点个数\tag{①} \\
        >   左子树结点个数+1=右子树结点个数\tag{②}
        > \end{align*}
        > $$

        因此对于剩余的那些结点
        * 如果判定树本身满足性质 $(①)$，插入该结点，就必须要满足性质 $(②)$。从根结点开始，逐层向下递归，应确保所有结点都满足性质 $(②)$
        * 如果判定树本身满足性质 $(②)$，插入该结点，就必须要满足性质 $(①)$。从根结点开始，逐层向下递归，应确保所有结点都满足性质 $(①)$
      ...
      * 示例
        ...
        假设需要构造一颗有 $n=10$ 个结点的判定树
        * $2^{3}-1\leq n=10\leq 2^4-1$，则 $h=3$，事先画出一颗 $h=3$ 的满二叉树。如图中前 $3$ 层
        * 剩余 $n-(2^h-1)=3$ 个结点，分别为 $8,9,10$ 号结点
        * 对 $8$ 号结点，插入前根结点都满足性质 $(①)$。因此从根节点开始，为了满足性质 $(②)$，根结点的右子树应该比左子树多一个结点，则 $8$ 号结点应该插入在根结点 $1$ 的右子树上。继续递归，查看根结点右子树的根结点 $2$ 号结点。仍然满足性质 $(①)$，应该插入到 $2$ 的右子树上。$2$ 的右子树的根结点 $4$ 仍然满足性质 $(①)$，则应该插入到 $4$ 的右子树上。最终 $8$ 插入到 $4$ 的右子树上了
        * 对 $9$ 号结点，插入前根结点都满足性质 $(②)$，则应插入到 $1$ 的左子树上，以使 $1$ 重新满足性质 $(①)$。按照刚才的递归方法，每个结点要么满足性质 $(①)$，要么满足性质 $(②)$。最终完成 $9$ 号结点的插入
        * 对 $10$ 号结点的操作同理

      * 总结
        * 当 $\text{mid}$ 向下取整(标准`int`特性)时，新结点总是倾向于插入某个结点的右子树
        * 当 $\text{mid}$ 向上取整时，则任意结点一定满足以下任意一条性质。插入方法同上，此时新结点总是倾向于插入某个结点的左子树

          $$
          \begin{align*}
            &左子树结点个数=右子树结点个数\tag{①} \\
            &左子树结点个数=右子树结点个数+1\tag{②}
          \end{align*}
          $$
...
* 平衡二叉树 $\text{AVL}$
  ...
  * 操作
    ...
    * 插入
      ...
      * $\text{LR}$ 型
        ...
        ![LR型](../../resource/image/data_struct/chapter5_query_AVL_LR.jpeg "LR型")
      * $\text{RL}$ 型

        以`B`为根的子树先对`C`做`LL`调整，以`A`为跟的子树再对`C`做`RR`调整

        ![RL型](../../resource/image/data_struct/chapter5_query_AVL_RL.jpeg "RL型")
    * 删除
      * 先删除目标结点，删除方法同二叉排序树
  ...
  * 性质
    * 深度为 $h$ 的平衡二叉树的最少结点数 $n_h=n_{h-2}+n_{h-1}+1$。其中，$n_0=0,n_1=1$
      > ***所有非叶结点的平衡因子的绝对值均为 $1\Leftrightarrow\text{AVL}$ 树结点最少***
...
* 红黑树 $\text{RBT}$
  * 定义
    ...
    * 根叶黑：根结点与失败叶结点均是黑色的
    * 不红红：上下不存在两个相邻的红结点
    * 黑路同：对任意结点，从该结点到任意叶结点的简单路径上(不含该结点自己)，所含黑结点的数量相同。黑结点的数量称为 **黑高**。特别地，叶结点指失败结点，也称空叶结点、NULL结点、外部结点；内部结点即为带值的那些结点

      > **示例**：图中结点`13`的黑高为 $2$

    ![红黑树](../../resource/image/data_struct/chapter5_query_RBT.jpeg "红黑树")
  * 性质
    * 从根结点到任意叶结点的最长路径长度不超过最短路径长度的 $2$ 倍。因此红黑树中左右子树的高度差不会超过 $2$ 倍

      > 黑高相同，路径最长时有黑高个红结点穿插其中，最短时没有红结点

    * 有 $n$ 个内部结点的红黑树高度 $h\leq2\log_2{(n+1)}$

      > $\log_2{(n+1)}$ 是全为黑结点时的二叉树树高，间隔补上红结点就是 $2\log_2{(n+1)}$

    * 若根结点的黑高为 $h$，则至少有 $2^h-1$ 个内部结点
    * $\text{AVL}$ 的查询效率优于 $\text{RBT}$，但二者的查找、插入、删除的时间复杂度都是 $O(\log_2{n})$
    * 红黑树插入时旋转的次数可能超过 $2$ 次
  * 操作
    * 查找
      * 同二叉排序树
    * 插入
      ...
      [完整插入过程](https://www.bilibili.com/video/BV1b7411N798?spm_id_from=333.788.videopod.sections&vd_source=c43b16e24e8afbeb40eac26ae4b482f9&p=80)
      * 时间复杂度是 $O(\log_2{n})$
    * 删除
...
* $\text{B}$ 树
  * 定义
    ...
    * 对任一结点，其左右子树高度相同。则 $\text{B}$ 树所有叶结点都在同一层
    ...
    * 删除
      * 非终端结点
        ...
        * 再将终端结点中目标关键字的直接前驱或直接后继移到被删除目标关键字的位置，相当于转化为删除终端结点
          * 直接前驱：概念同线索二叉树，在目标关键字左指针的最右下，**是目标关键字所在结点的【整个左子树】中最大的关键字**
          * 直接后继：概念同线索二叉树，在目标关键字右指针的最左下，**是目标关键字所在结点的【整个右子树】中最小的关键字**
          > 删除任意关键字一定转换为删除终端结点中的关键字
      * 终端结点
...
* 性能分析(以线性探测法为例)
  ...
  * 聚集现象：多个初始散列地址不同的元素(即非同义词)在处理冲突时争夺同一个地址
