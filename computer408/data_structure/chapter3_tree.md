...
* 遍历
  ...
  * 森林
    ...
    * **森林的中序遍历也称森林的后序遍历、后根遍历、中根遍历。森林只有先序和中序两种遍历方法，后序是不同教材上对中序的不同叫法**
...
* 遍历
  ...
  * ***线索二叉树***
    ...
    * ***前驱与后继线索***
      ...
      |**后继**|有左孩子则为左孩子</br></br>没有左孩子则为右孩子|右子树最左下结点|如果是根节点，则没有后继</br></br>如果是右孩子，则为父节点(需要三叉链表)</br></br>如果是左孩子且右子树为空，则为父节点(需要三叉链表)</br></br>如果是左孩子且右子树不为空，则为后序遍历右子树的最后一个结点|
...
* 哈夫曼树与哈夫曼编码
  * 概念
    ...
    * 设 $n$ 为叶节点的个数，则
      ...
      \begin{align*}
        树的带权路径长度\text{WPL}
        &=\sum^n_{i=1}{\big(叶结点的权值\times根结点到该叶结点的路径长度\big)} \\
        &=树中所有非叶结点的权值之和
      \end{align*}
      $$
  ...
  * 构建哈夫曼树
    ...
    * 重复上述操作，直至队空
  * 哈夫曼树性质
    * 有 $n$ 个叶结点的哈夫曼树的结点总数为 $2 n-1$

      > 除最开始的两个结点外，其余每个结点加入时都会生成一个新的结点，因此结点总数为 $2(n-2)+3=2 n-1$

    * 哈夫曼树中不存在度为 $1$ 的结点
    * 哈夫曼树不唯一，但 $\text{WPL}$ 唯一
    * 每个初始结点最终都会成为叶结点，且权值越小的结点离根结点越远
  * 哈夫曼编码
