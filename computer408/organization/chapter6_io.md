...
* 接口结构
  ...
  * 外部接口(设备侧)：外部接口通过 **接口电缆(通信总线)** 与外设相连，外部接口的数据传输可能是串行方式，因此 $\text{I/O}$ 接口需具有串并转换功能
  * ⚠️**命令字(控制字)、状态字、中断类型号都用数据总线传输；中断请求用控制总线传输**
  * ⚠️**主机与外设间的数据通路：$\text{CPU}$ 和主存 $\to$ $\text{I/O}$ 总线 $\to$ $\text{I/O}$ 接口(外设控制器) $\to$ 接口电缆(通信总线) $\to$ 外设**
  * I/O接口的作用
...
* 接口类型
  ...
  * **磁盘驱动器是 $\text{I/O}$ 设备，不是 $\text{I/O}$ 接口**
  * **打印机适配器**用来适配计算机和打印机的数据转换，属于 $\text{I/O}$ 接口
  * **网络控制器**俗称网卡，用来适配计算机与网络通信数据，属于 $\text{I/O}$ 接口
  * **可编程中断控制器**用来识别和协调外设的中断通信，属于 $\text{I/O}$ 接口
...
* 程序中断方式
  ...
  * 中断流程
    ...
    * 中断处理
      ...
      * 中断服务程序
        ...
        隐指令处理结束后，在运行中断程序前先保护现场，把各类寄存器的值压入栈顶，然后再执行中断服务。执行结束后恢复现场，把各类寄存器的值再写回原样，同时通过保存的 $\text{PC}$ 值回到断点
  ...
  * $\text{I/O}$ 程序中断方式(前边是系统中断基础概念)
    ...
    * 中断响应阶段([中断隐指令](../_extra/ep3_interrupt_implicit_instruction.md))
      * 关中断
      * 保存**断点 $(\text{PC})$、程序状态字 $(\text{PSW})$**
      * 根据**响应优先级**确定响应顺序，其中响应优先级由 **硬件电路** 确定
      * 送中断向量
    * 中断执行阶段(中断服务程序)
      * 保存**现场( 通用寄存器组 $\text{GPRS}$、设置断中断屏蔽字 )**，其中中断屏蔽字决定**执行优先级**。这些工作由 **软件程序(操作系统)** 完成
      * (多级中断：开中断)
      * 执行中断服务程序
      * (多级中断：关中断)
      * 恢复现场
...
* $\text{DMA}$ 方式
  * 流程
    ...
    > 预处理阶段 $\text{CPU}$ 给 $\text{DMAC}$ 对应的主存地址、外设地址、数据计数，然后 $\text{DMAC}$ 开始工作。每准备好一个数据单元， $\text{DMAC}$ 就尝试发起一次 $\text{DMA}$ 传输请求，请求占用总线向主存写入数据， $\text{CPU}$ 在存取周期的末尾让出总线给 $\text{DMA}$ 请求，这样一次数据就写完了，地址对应更改，数据计数-1.往返多次直至整块数据写完，随后 $\text{DMAC}$ 发出中断信号， $\text{CPU}$ 执行后处理程序服务进行收尾工作。整体而言可以近似看作DMA方式只在最后完成传输时发出了一次中断请求，其余时间传输数据不影响 $\text{CPU}$ 工作，因为窃取的是存取周期并非指令周期，$\text{CPU}$ 在被窃取时仍然可以执行指令，只是无法访存而已。$\text{DMA}$ 工作方式详见[DMA工作方式](../_extra/ep5_DMA.md)
...
* 总结
  ...
  ![IO方式总结](../../resource/image/organization/chapter6/io_mode_compare.png "IO方式总结")
