...
* 组成
  ...
  * 电容充电写入数据，电容放电读出数据
  * ⚠️**存储芯片对外暴露的金属引脚数量=地址线数量+数据线数量+片选线(1根)+读写控制线(1或2根)**
    * 地址线数量= $\text{MAR}$ 的位数
    * 数据线数量= $\text{MDR}$ 的位数
    ...
    * **芯片规格可写作`存储单元数*数据位数`的样式。如 $\text{1K}\times 8$ 芯片有 $10$ 根地址线，$8$ 根数据线**
...
* $\text{SRAM}$、$\text{DRAM}$
  ...
  * 刷新
    ...
    * 采用行列地址减少地址线数量，地址线分别对接行地址译码器和列地址译码器。如地址有 $8$ 位的情况下，使用一维地址要 $8$ 根地址线来表示 $2^8=256$ 个地址；使用行列地址各需要 $4$ 根地址线即可表达 $2^4\cdot 2^4=256$ 个地址
...
* 双端口 $\text{RAM}$、多模块存储器
  ...
  * 多模块存储器
    * 多体并行存储器
      ...
      * 低位交叉编址效率更高，因为数据大多存储在一片连续的存储单元中，读取下一个存储单元的数据时可以避开上一个存储单元的存储体的恢复时间。**若总线带宽为 $k\text{bit}$，则 $m$ 体低位交叉编址一个存取周期 $T$ 内的总带宽为 $(km)\text{bit}$**。总线带宽计算详见[总线带宽](../_extra/ep6_bus_bandwidth.md)
...
* 磁盘
  ...
  * 廉价冗余磁盘阵列 $\text{RAID}$
    ...
    **$\text{RAID3}$ 只将奇偶校验信息放在一个磁盘上，磁盘损坏则失去校验功能；$\text{RAID5}$ 用多个磁盘存放奇偶校验信息，即使一个磁盘损坏也能提供校验功能**
...
* 固态硬盘 $\text{SSD}$
  ...
  * $\text{SSD}$ 比磁盘更容易磨损，因此要采用磨损均衡技术延长寿命
...
* ⚠️ $\text{Cache}$ 容量
  ...
  | **有效位** | $1$ | 必须存在 |
  | **脏位** | $1$ | 使用写回法时存在 |
  | **替换控制位** | $k$ 路组相联：$\lceil \log_2{k} \rceil$ 位 $/$ 组 | 只有在使用 LRU 替换策略时才需要 |
