...
* 加法器
  * 加法器电路图
    ...
    * $\text{OF}=C_n\oplus C_{n-1}=\overline{C_n}C_{n-1}+C_n\overline{C_{n-1}}$。如果是正数相加，若最高次高均未进位，则符号位正常仍未0，没有溢出；如果是负数相加，最高次高均进位，但次高的进位 $1$ 补充到符号位上了，符号仍然是 $1$。没有溢出。反之若 $\text{XOR}$ 得到了 $1$，说明正数相加得到了负数或负数相加得到了正数，显然是溢出了
...
* 定点数乘除运算
  ...
  * 带符号数乘运算
    ...
    也称“补码一位乘法”，由Booth提出，也称“布斯(Booth)乘法”
...
* 浮点数定义
  * 单精度浮点数 $\text{float32}$
    ...
    * 阶码采用的移码偏置值为 $2^{n-1}-1=127$。其中，$n=8$ 为阶码的位数
      ...
      | 实际可用的正常阶码范围 | $[-126,127]$ |
      | 极端阶码用途 | `-127(00000000)` $\to$ 非规格化数或零 </br> `+128(11111111)` $\to\pm\infty$ 或 NaN |
  ...
  * 双精度浮点数 $\text{double64}$
    ...
    * 阶码采用的移码偏置值为 $2^{n-1}-1=1023$。其中，$n=11$ 为阶码的位数
      ...
      | 极端阶码用途 | `-1022(00000000000)` $\to$ 非规格化数或零 </br> `+1023(11111111111)` $\to\pm\infty$ 或 NaN |
...
* 边界对齐
  ...
  * `字`=`4Byte`=`32bit`，`半字`=`2Byte`=`16bit`，每次 $\text{IO}$ 只能以`字(4Byte)`为单位
