...
* 定点数乘除运算 ([溢出机制详解](../_extra/ep9_imul_umul.md))
  * 原码乘运算
  ...
  * 无符号数除运算
    ...
    * $\text{ALU}$ 需要实现加法和减法
    * 当除零时会产生异常。此外，在带符号数除法中，最小的负数除 $-1$ 也会发生异常。如 $7$ 位补码最小的负数为 $-128$，但 $-128/-1=128$，超出 $7$ 位补码表示范围
...
* 浮点数加减运算
  ...
  ![IEEE754加减运算](../../resource/image/organization/chapter1/chapter1_digit_IEEE754_plus.png "IEEE754加减运算")

  1. 对阶，小阶看大阶。不用大阶看小阶是避免大阶对小阶时尾数上溢
  2. 尾数进行无符号数加减运算
  3. 运算后的尾数规格化，调整尾数及阶码
...
* 边界对齐
  ...
  * ⚠️**边界对齐方式下变量的始址必须是其大小的整数倍**
  * 边界对齐方法空间换时间，每次 $\text{I/O}$ 都能获得完整数据；边界不对齐方法时间换空间，末尾可能出现数据分开存在两个`字`的情况，读取需要两次 $\text{I/O}$
