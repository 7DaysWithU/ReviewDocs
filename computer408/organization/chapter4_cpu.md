...
* $\text{CPU}$
  ...
  * ⚠️ **$\text{ALU}$ 的 $\text{B}$ 端连接的 $\text{MUX}$ 中，可以将 $\text{MUX}$ 的一个接入设置为一条指令占的存储单元数，方便 $\text{PC}$ 执行 $\text{PC}+1$ 操作**
...
* 指令执行方案
  ...
  * **单周期CPU不能采用单总线结构数据通路。因为一个时钟周期总线只能有一次数据流，而单周期指令要在一个时钟周期完成所有操作，会有多次总线数据流**
  * **单周期CPU在指令执行过程中控制信号不变**
...
* 数据通路组成
  ...
  * 从元件上看
    ...
    数据通路由组合逻辑元件(操作元件)、时序逻辑元件(状态元件)组成
    * **组合逻辑元件(操作元件)**：不含存储信号的记忆单元，也不受时钟信号控制，输入输出间无反馈通路。包括 $\text{ALU}$ 、加法器、译码器、多路选择门 $\text{MUX}$、三态门等
    * **时序逻辑元件(状态元件)**：含存储信号的记忆单元，输出与输入时间序列有关，元件必须在时钟节拍控制下工作。包括各类寄存器和存储器，如通用寄存器组 $\text{GPRS}$、$\text{PC}$、$\text{IR}$、$\text{PSW}$ 等

  ⚠️**特别地，数据通路中包含用于异常事件检测及响应的电路**
