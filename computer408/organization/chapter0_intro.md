...
* 软件
  ...
  * 语言级别
    ...
    * 高级语言：现代常用编程语言，如 $\text{C}、\text{Python}$
      * 使用 **解释器 (执行一句翻译一句)** 翻译成机器语言，如 $\text{Python}、\text{JavaScript}、\text{Shell}$。***不会生成目标代码文件***
      * 使用 **编译器 (先翻译再执行)** 先翻译成汇编语言，再翻译成机器语言；或直接翻译成机器语言，如 $\text{C++}$。***会产生目标代码文件。执行流程：预处理 $\to$ 编译 $\to$ 汇编 $\to$ 连接***
  * 软件与硬件
...
* 其他
  * 机器字长 (详见[字长与位宽](../_extra/ep10_word_length.md))
