...
* 基本分页存储管理
  ...
  * 基本地址变换机构
    ...
    * 页表长度`M = len(page_table)`，表示页表项的数量。仅当要访问的页号`P`小于`M`时才不会溢出，即`0 <= page_id <= len(page_table)) - 1`
    * $\text{MMU}$ 是页式虚拟存储系统中的硬件地址翻译器，核心功能是将虚拟地址转换为物理地址，并在转换过程中自动完成页号越界检查、缺页检测和 $\text{TLB}$ 查询/缺失处理
    * **根据逻辑地址查找页表的过程由硬件(页表寄存器 $\text{PTR}$、内存管理单元 $\text{MMU}$ )实现**
...
* 概念
  ...
  * 容量与寻址空间
    ...
    **实际能用多大存储？** $\to$ 由物理内存 + 磁盘 swap 空间决定（虚拟存储容量）
    ...
    $$
    虚拟内存最大容量=\min(内存容量+外存容量，\text{CPU}最大寻址范围)
    $$

    所以前者是“逻辑范围”，后者是“物理资源”，**虚地址空间可以远大于实际存储，但不能超过它太多，否则无法有效利用**。

    ⚠️**由于需要根据实际情况对页面调入调出，所以虚拟存储只能基于非连续分配方式，且实际容量受内存容量和外存容量共同决定。但虚地址空间不直接受内存容量和外存容量所影响**
...
* 请求分页管理方式
  ...
  ```C
  struct TLB {
    unsigned int tag: 20;           // 虚拟页号（VPN）
    unsigned int pfn: 20;           // 物理页框号（PFN）
    unsigned int valid: 1;          // 是否有效（请求页式关键）
    unsigned int dirty: 1;          // 是否被修改
    unsigned int referenced: 1;     // 是否被访问
    unsigned int protection: 3;     // 读/写/执行权限
    unsigned int asid: 8;           // 进程ID（可选）
    unsigned int global: 1;         // 是否全局共享
    // 其他控制位...
  };
  ```

  * $\text{TLB}$ 必须带虚拟页号 $(\text{Tag})$，因为可能存在页不在 $\text{TLB}$ 中，不能像页表那样模拟连续的地址
  ...
  * 中断类型属于内中断—故障，可被修复
  ...
  ```mermaid
  graph TD
    A[CPU 发出虚拟地址] --> B[提取虚拟页号和页内偏移]
    B --> C{查询 TLB}
    C -- TLB 命中 --> D[从 TLB 获取物理页框号]
    C -- TLB 未命中 --> E[访问页表, 查找页表项]
    E --> F{页表项有效位 = 1?}
    F -- T --> G[从页表获取物理页框号]
    F -- F --> H[触发缺页中断]
    H --> I[调入页面至内存]
    I --> J[更新页表项，设置有效位, 填写物理页框号]
    J --> K[更新 TLB]
    K --> L[重新执行原指令]
    L --> C
  ...
  * 访问不在内存的页：访问 $\text{TLB}$ $1$ 次 $+$ 访问页表 $1$ 次 $+$ 就缺页中断访磁盘 $1$ 次 $+$ 访问 $\text{TLB}$ $1$ 次 </br> 访问在内存但不在 $\text{TLB}$ 的页：访问 $\text{TLB}$ $1$ 次 $+$ 访问页表 $1$ 次 </br> 访问在内存且在 $\text{TLB}$ 的页：访问 $\text{TLB}$ $1$ 次 </br> **经历上述访问次数后即可拿到页的物理地址，根据物理地址再去访存即可获得页面内容**
...
* 页面置换算法
  ...
  * 时钟置换算法 $\text{CLOCK}\,/$ 最近未使用算法 $\text{NRU}$
    ...
    * 初始访问位为`1`
    * 需要淘汰时，从队首开始扫描，访问位为`1`的变成`0`，访问位为`0`的淘汰
...
* 3.1习题
  * ⭐***11(页表项位数与页面页号的关系、多级页表获得页号、计算物理地址、十六进制与二进制加法)***
* 3.2习题
  * ⭐***14(虚地址转逻辑页号、虚拟页式访存时间、LRU夺舍物理页框)***
  * 15(给逻辑地址算物理地址、FIFO置换后的物理地址、CLOCK置换后的物理地址)
  * 16(页框回收策略之页面缓冲算法)
  * ***17(分页式虚拟地址空间大小、二级页表的页表占的页数、低级页表存的是高级页表的地址)***
  * ***18(机器指令、二级请求页表、CPU状态、进程状态)***
  * ***19(写出十六进制虚地址、PDBR保存页目录始址的物理地址、线程共享进程的资源、改进型CLOCK)***
  * 20(根据页框号算页框物理地址)
  
    > $物理地址=始址+页框号\times 页大小+页内偏移量$。其余地址计算方法大同小异

  * ⛔***21(页表项的虚地址实地址页框号、根据页框物理地址算页框号)***
  
    > 页表作为进程的一部分，其自身也会存储页表占用的页的页表项，即页表本身也是一种数据
