...
* $\text{I/O}$ 软件层次结构
  ...
  * 设备驱动程序负责将上层软件发来的抽象 $\text{I/O}$ 要求转换为具体要求，发送给设备控制器，控制设备工作。设备驱动程序需要向设备寄存器写入命令，以控制设备的工作状态和数据传输方式。**因此驱动程序与 $\text{I/O}$ 控制方式相关**
...
* 假脱机技术 $\text{SPOOLing}$
  ...
  * 采用 $\text{SPOOLing}$ 技术的计算机不需要专门的外围控制机，因为 $\text{SPOOLing}$ 中的输入进程、输出进程采用软件模拟实现了外围控制机，**因此 $\text{SPOOLing}$ 技术需要多道程序设计技术的支持**
...
* 磁盘
  ...
  簇号转换磁盘地址详见[簇号转换磁盘地址](../_extra/ep7_cluster_to_disk.md)
...
* 磁盘管理
  ...
  * **逻辑格式化也称高级格式化。在此期间文件系统被创建，操作系统存储引导程序，文件系统的索引结点表被创建**
  * ⚠️物理格式化 $\to$ 磁盘分区 $\to$ 逻辑格式化 $\to$ 安装操作系统
...
* 磁盘调度
  ...
  * 调度时间
    ...
    * 采用`(驱动器号, 柱面号, 盘面号, 扇区号)`的地址格式时，当一个盘面的所有扇区读完时，下一个连续地址就是下一个盘面的初始扇区。同交替编号的思想一样，为避免处理数据时转过该读的扇区，不同盘面间也错位命名，使上下同一位置的扇区号不一样，留出时间进行数据处理
