...
* ⚠️操作
  ...
  * **`create()`操作创建新文件`f`时，会对`f`的索引结点初始化，在目录文件中写入`f`的索引结点号，并在目录文件中增加一条`f`对应的目录项，最核心的信息——记录在目录文件里——目录文件只存文件名和索引结点编号。文件`f`更详细的信息则存放在其索引结点中，例如`f`的访问权限信息等**
  * **`open()`系统调用实现了文件按名存取功能**
  * **`read()`系统调用只需要文件描述符`fd`而不需要文件名，是因为`read()`之前已经使用过`open()`系统调用根据文件名将文件的 $\text{FCB}$ 读入到内存的打开文件表里了，所以读操作只需使用`fd`指针即可，不需要反复`open()`**
...
* 文件共享
  ...
  * 所有用户共享同一个索引结点`inode`
  * ⚠️**修改链接计数变量`count`的值是在调入内存的索引结点中修改的，不是在磁盘中的索引结点中修改的。因为使用`count`意味着文件被访问，显然此时文件的索引结点应该已经被调入内存了**
  
  ![文件共享软链接](../../resource/image/os/chapter3/file_share_soft_link.png "文件共享软链接")

  * 软链接的`count`值不受被链接文件的影响，无论被链接文件存不存在，软链接计数始终为 $1$
  * 软链接是一个独立的文件，拥有**自己的`inode`和目录项**。其文件内容是目标文件的路径。当打开软链接时，系统会解析该路径并打开目标文件。因此，**软链接与目标文件的`inode`完全无关；它们是两个不同的文件，只是通过路径名建立逻辑关联。**
...
* 文件控制块 $\text{FCB}$ 与索引节点
  ...
  * 老式操作系统中，文件目录项就是 $\text{FCB}$，因为目录就是 $\text{FCB}$ 的集合

  ![索引结点](../../resource/image/os/chapter3/directory_index_node.png "索引结点")

  * $\text{FCB}$ 与目录项关系

    在 $\text{UNIX}$ 文件系统中，**索引结点（inode）就是文件控制块（FCB）**，用于存储文件的**元数据**，包括：文件类型、访问权限、大小、时间戳、以及指向数据块的指针（如起始块号、块数等）。
  
    文件的实际内容可能分散存储在多个磁盘数据块中。  
  
    为了支持按名存取，系统在**目录中建立精简的索引项（目录项）**，每项仅包含 **文件名和对应的 inode 编号**。  
  
    通过文件名查目录项得到 $\text{inode}$ 编号，再读取 $\text{inode}$ 即可获取文件全部属性和数据位置。
  
  * 因此，`open()`系统调用需要文件名从目录项里找到对应的 $\text{FCB}$，并把 $\text{FCB}$ 读入内存，这样就知道文件内容从哪开始，有多长等信息，预备着给`read()`使用。使用`read()`系统调用时，因为 $\text{FCB}$ 已经在内存了，也就是按名存取这一步已经完成了，所以只需要说明想读哪？读多少？读出到哪里这些参数——即不需要传输文件名，这时才会真正地把文件内容读入到内存中
...
* 文件系统布局
  ...
  * ⚠️ **进程读取文件(`read`系统调用)只需要文件描述符，不需要其他任何参数(比如路径、文件名等)**
...
* 存储空间管理
  ...
  * 位示图法
    ...
    * 连续、离散分配都适用
    * **位示图的大小与空闲块的数量无关。因为位示图需要标记所有块，无论已分配还是未分配**
