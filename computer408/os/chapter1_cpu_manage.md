...
* 进程通信
  ...
  * 信号
    ...
    | 信号名 | 产生原因 / 触发条件  | 默认处理动作  | 考察要点说明 |
    |-------|----|-----------|------|
    | **SIGINT**   | 用户中断（如按 `Ctrl+C`）     | 终止进程             | 最常见交互式中断信号 |
    | **SIGQUIT**  | 用户退出（如按 `Ctrl+\`）  | 终止进程 + 生成 core dump | 与 SIGINT 区别：会生成 core 文件 |
    | **SIGKILL**  | 强制终止进程（`kill -9`）  | **立即终止，不可被捕获或忽略** | **重点！** 不可被 signal() 或 sigaction() 捕获 |
    | **SIGTERM**  | 请求终止（`kill` 默认信号）   | 终止进程（可被捕获） | 可用于优雅关闭程序 |
    | **SIGSTOP**  | 暂停进程（`kill -STOP` 或 `Ctrl+Z`）| **暂停进程，不可被捕获** | 与 SIGKILL 类似，不可被忽略或捕获 |
    | **SIGCONT**  | 继续已暂停的进程    | 恢复执行             | 与 SIGSTOP 配对使用 |
    | **SIGALRM**  | 定时器超时（由 `alarm()` 系统调用触发）     | 终止进程             | 常与定时、超时控制结合考察 |
    | **SIGCHLD**  | 子进程状态改变（终止或暂停） | 忽略（默认）         | 用于避免僵尸进程（父进程可捕获处理）|

    * 收到重复的信号直接简单丢弃
    * 进程可以给自己发信号
    * 不能被捕获：即不能由用户自定义处理程序，必须使用系统原装版本
...
* 资源分配
  ...
  | **只读数据区 </br> （RO Data）** | 进程 | ✅ 共享 | 存放`const`常量、字符串字面量（如 `"Hello"`）等 |
  | **可读写数据区 </br> （Data & BSS 段）** | 进程 | ✅ 共享 | <ul><li>**Data 段**：已初始化的全局变量（如 `int a = 1;`）和静态变量`static`</li><li>**BSS 段**：未初始化或初始化为 0 的全局/静态变量</li></ul>所有线程共享 |
  ...
  * **父进程使用`fork()`创造子进程，随后二者就相当于两个不同的独立进程，有各自独立的 $\text{PCB}$ 。二者不共享虚拟地址空间，访问同一临界资源需互斥访问**
  * **$\text{PCB}$ 存储在内核区**
...
* 概念
  ...
  * 同一个互斥资源的临界区只能有一个进程进入。哲学家就餐问题中每根筷子构成一个独立的临界区
  * 让权等待不是强制要求，可以不满足。但空闲让进、忙则等待、有限等待必须满足
...
* 实现
  ...
  * 硬件实现
    ...
    ⚠️**由硬件实现的原子操作，其原理为关闭总线，而非关中断**。使用 `SWAP`、`TSL`、`LOCK` 等硬件指令实现的互斥机制（如自旋锁），**不满足“让权等待”原则，也没有实现阻塞-唤醒机制**。它们依赖进程**主动循环检测**锁状态，一旦锁释放，下一个循环就能进入
...
* 信号量
  * 分类
    ...
    * 记录型信号量当 $\text{value}>0$ 时，表示有 $\text{value}$ 个资源 </br> 记录型信号量当 $\text{value}=0$ 时，表示资源正好分完，且没有进程等待 </br> 记录型信号量当 $\text{value}<0$ 时，共有 $\lvert\text{value}\rvert$ 个进程在等待该资源
...
* 经典问题
  * 生产者消费者问题
    ...
    * 虽然对`full`的初值没有上限限制，但生产者想生产就得先`P(empty)`，如果缓冲区满，则生产者因为无法获得`empty`进而被阻塞，也就没法生产。因此即使`full`没有上限限制也不会导致缓冲区容量爆炸。同理，消费者想拿需要先`P(full)`，缓冲区空就没办法拿，也就没办法让缓冲区容量变成负的。两个信号量彼此互为上下限，通过同步操作规范了生产者和消费者的越界行为
  ...
  * 读写者问题
    ...
    ![读写者问题](../../resource/image/os/chapter1/semaphore_classic_reader_writer.png "读写者问题")
    ...
    * 使用计时器`count`对读进程计数，第一个读进程上读写锁`rw`，最后一个读进程释放读写锁`rw`，中间来的读进程直接可以读。既满足读进程与写进程互斥访问临界区，也满足多个读进程可以同时访问临界区。但由于`count`的检查与修改不是原子操作，可能出现多个读进程同时申请了读写锁`rw`导致读进程互斥访问的问题，因此再使用`mutex`实现对`count`操作的互斥访问，确保同时只能有一个读进程对计数器`count`操作，避免对`rw`的重复操作
  ...
  * 哲学家进餐问题
    ...
    * 其他思路 (详见[哲学家进餐问题](../_extra/ep0_philosopher_delimma.md))
  ...
  * 总结
    ...
    * 读写者问题属于复杂互斥问题。求解关键是分清不同的互斥资源，明确互斥之间的优先级，若存在多个进程访问且有并发要求时使用`count`进行计数，根据计数器的不同值进行响应操作，同时计数器也应该互斥访问
    ...
    * 代码模板
      ...
      cobegin{              // 并行开始
        ...
        Process P2()        // P2进程(一次性进程不需要永真循环，如只来一次的顾客进程)
          P(mutex);
          ...
          V(mutex);
      }coend                // 并行结束
...
* 管程
  ...
  * ⚠️管程与`V`原语(即`signal(s)`原语)对比
    ...
    ***管程中的`signal`操作与`V`原语(即`signal(s)`)并不相同***
    ...
    > * 进程在条件不满足时调用 `x.wait()` 进入等待队列，变成**阻塞态**
...
* 概念
  ...
  ![死锁](../../resource/image/os/chapter1/deadlock.png "死锁")
...
* 死锁避免
  ...
  * ⚠️**安全状态一定不会发生死锁，发生死锁一定是不安全状态，不安全状态不一定发生死锁**

  ![银行家算法](../../resource/image/os/chapter1/deadlock_avoid_banker.png "银行家算法")

  * 银行家算法要求，进程运行之前先声明它对各类资源的最大需求量，并保证它在任何时刻对每类资源的请求量不超过它所声明的最大需求量。当进程已占有的资源数与本次申请的资源数之和不超过对资源的最大需求量，且现存资源量能满足所需的最大资源量时，才分配资源
  * ⚠️**银行家算法是"死锁避免"而不是"死锁预防"，且没有破坏任何一个死锁必要条件。银行家算法只能判断系统是否处在不安全状态，检测不到死锁。检测死锁是"死锁检测"独有的功能**
...
* 死锁检测与解除
  ...
  * ⚠️**死锁检测算法不需要进程运行所需的资源总量信息，但死锁避免(银行家算法)需要**

  ![死锁检测算法](../../resource/image/os/chapter1/deadlock_detect_algorithm.png "死锁检测算法")

  * 本质上与银行家算法的分配检测一样，都是检测能否给一个进程分配资源，使其运行并释放所占用的资源。不过银行家算法是死锁避免，而资源分配图是已经死锁了——用来检测哪些进程死锁了

  ![死锁解除算法](../../resource/image/os/chapter1/deadlock_unfreeze_algorithm.png "死锁解除算法")

  * 资源剥夺法通常只从死锁进程抢占资源，而不从非死锁进程抢占资源
...
* 2.3习题
  * ***17(生产者消费者问题、多种产品)***
    ...
    > 只要合理确定资源总体的上下界，单个资源缺少单个边界不会影响生产者消费者问题。如`full = 0`不会因为没有上界导致缓冲区溢出
  
  * ⭐***18(生产者消费者问题、复杂同步互斥问题)***
  * 19(生产者消费者问题)
  * ***20(生产者消费者问题、FCFS锁`rw`)***
  * ***21(生产者消费者问题、双缓冲区)***
  * ***22(读写者问题、多资源的简单解法)***
  
    > * 追求公平就用 $\text{FCFS}$ 锁`rw`；追求简单直接叠互斥信号量就行
    > * **为什么能叠互斥信号量：**$\text{P1}$ 与 $\text{P2}$ 关于 $Y$ 互斥，$\text{P2}$ 与 $\text{P3}$ 也关于 $Y$ 互斥。**虽然这两对进程都是关于 $Y$ 互斥，但并不是同一个互斥关系，而是两个互斥关系，应分别设互斥信号量 $\text{mutexY}_\text{12}$ 和 $\text{mutexY}_\text{23}$ 进行使用**。所以在写进程里，可以对同一个变量简单叠加互斥信号量。如写进程修改 $Y$ 需要同时具备 $\text{mutexY}_\text{12}$ 和 $\text{mutexY}_\text{23}$

  * ***23(哲学家进餐问题)***
  * 24(前驱关系)
  * ⭐***25(用开关中断指令实现的PV操作)***
  * 26(前驱关系)
  * ⭐***27(SWAP实现的PV操作、同时读写互斥变量导致失去互斥性)***
  * ***28(只执行一次的生产者消费者进程)***
* 2.4习题
  * ***04(死锁公式)***
  * ***05(死锁预防与避免方法)***
