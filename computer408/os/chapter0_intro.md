...
* ⚠️**单道批处理类似单指令，上一个作业处理完才能开始下一个作业；多道批处理类似指令流水线，支持进程并发执行**
* 多道批处理系统(多道程序系统)不必实现虚拟存储管理。这是早期技术问题，现在虚存负责的存储管理当年是让程序员手动管理的
* 中断技术使得多道批处理系统的 $\text{I/O}$ 设备可与 $\text{CPU}$ 并行工作
...
* 中断与异常
  ...
  * 内中断需要在指令执行过程中立即响应。外中断中优先级低的统一在指令周期末尾的中断周期处理(如可屏蔽中断，时钟中断、$\text{I/O}$ 中断)，优先级高的可能需要立即处理(如不可屏蔽中断)
  * **故障**返回到引起故障的指令**重新执行**；**陷阱**返回到自陷指令的**下一条指令**；**终止**直接**终止执行**
...
* 系统调用
  ...
  * **页面置换、进程调度是操作系统自动完成的，不需要用户手动使用系统调用。但有时创建新进程需要用户自己创建(如自己编程使用进程池)，这时候就需要使用系统调用`fork()`**
  * 系统调用过程中，由 **操作系统(软件)** 完成的是保存通用寄存器组 $\text{GPRS}$、执行系统调用服务例程。保持断点 $(\text{PC})$ 和程序状态字 $\text{PSW}$、将 $\text{CPU}$ 转内核态是 **硬件** 执行的。

  ![系统调用过程](../../resource/image/os/chapter0/system_call_procedure.png "系统调用过程")

  > ⛔ ***( 2023 统考 T46 )( 2024 统考 T31 )以进程 $P$ 通过执行系统调用从键盘里接收一个字符的输入为例，说明系统调用的执行流程***
  >
  > 将进程 $P$ 插入阻塞队列 $\to$ 用户在键盘上输入字符 $\to$ 启动键盘中断处理程序 $\to$ 将字符从键盘控制器读入系统缓冲区(即**内核缓冲区**) $\to$ 将进程 $P$ 插入就绪队列 $\to$ 进程 $P$ 从系统调用返回
  >
  > ⚠️更详细的过程涉及**内核缓冲区和用户缓冲区**，详见 **26王道-操作系统-5.2.6(P327开始)**

  * 系统调用流程：传参 $\to$ $\text{trap}$ 自陷 $\to$ 用户态转核心态 $\to$ 执行服务程序 $\to$ 核心态转用户态 $\to$ 调用返回
