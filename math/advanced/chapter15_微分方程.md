...
* 二阶常系数齐次线性微分方程
  ...
  对于方程 $y^{''}+py^{'}+q=0$，其中 $p,q$ 为常数，方程的特征方程为 $r^2+pr+q=0$。则方程的通解为
...
* 二阶常系数非齐次线性微分方程
  ...
  其中，$y(x)$ 为方程 $y^{''}+py^{'}+q=0$ 的通解，$y^*(x)$ 为二阶常系数非齐次方程的特解，特解可由两种方法得到。其中，待定系数法在设出 $y^*$ 后需要将特解带回方程求解系数，微分算子法直接求解设出的 $y^*$ 即可。特别地，有
  > 1.方程 $y^{''}+py^{'}+q=f_1(x)+f_2(x)$ 的特解为 $y^{''}+py^{'}+q=f_1(x)$ 的特解加 $y^{''}+py^{'}+q=f_2(x)$ 的特解
  >
  > 2.方程 $y^{''}+py^{'}+q=f(x)$ 的通解为 $y^{''}+py^{'}+q=f_1(x)$ 的特解减 $y^{''}+py^{'}+q=f_2(x)$ 的特解
